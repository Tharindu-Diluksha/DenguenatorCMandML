...
write.csv(optimumvalues_ml, file = "optimumValues_ml.csv", append = T, row.names = F, col.names = F, sep = ",")
...
featureReduction = function(reductionCriteria) {
  ...
  ## Correlation
  if(reductionCriteria == 1) {
    correlation = as.data.frame(abs(cor(data.frame(trainingDataFrame[,!(names(trainingDataFrame) %in% c("moh_name", "population", "year", "cases"))]), trainingDataFrame$cases)))
    correlation = data.frame(feature = row.names(correlation), correlation = correlation$V1, stringsAsFactors = F)
    
    selectedRows <<- c(correlation[correlation$correlation > round(median(correlation$correlation), 3),]$feature, "cases", "moh_name", "year", "population")
  }
